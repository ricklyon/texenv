...
class TestEmbeddedMacros(unittest.TestCase):
    ...
    def test_macro_args(self):
        ...
        filepath = dir_ / "kwargs.tex"
        texpp = TeXPreprocessor(filepath)
        texpp.run()

        pp_filepath = dir_ / "build/kwargs.tex"

        with open(pp_filepath) as pp_file:
            pp_text = pp_file.read()

        pp_filepath = dir_ / "kwargs_truth.tex"
        ...
        sync_pp = np.load(self.build_dir / "kwargs.syncmap.npy")
        np.testing.assert_array_equal(sync_pp, np.arange(1, 14))
